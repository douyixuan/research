...
RQ2_FILES = {
    "hdd_ddmin_c": "hdd_ddmin_c.csv",
    "hdd_wdd_c": "hdd_wdd_c.csv",
    "perses_ddmin_c": "perses_ddmin_c.csv",
    "perses_wdd_c": "perses_wdd_c.csv",
    "hdd_ddmin_xml": "hdd_ddmin_xml.csv",
    "hdd_wdd_xml": "hdd_wdd_xml.csv",
    "perses_ddmin_xml": "perses_ddmin_xml.csv",
    "perses_wdd_xml": "perses_wdd_xml.csv",
}

# Artifact README Table-1 summaries. Values are rounded there.
EXPECTED_TABLE = {
    "hdd_ddmin_c": {"time": 39108.0, "size": 518.0},
    "hdd_wdd_c": {"time": 18022.0, "size": 477.0},
    "perses_ddmin_c": {"time": 4582.0, "size": 281.0},
    "perses_wdd_c": {"time": 4169.0, "size": 278.0},
    "hdd_ddmin_xml": {"time": 3152.0, "size": 98.0},
    "hdd_wdd_xml": {"time": 2621.0, "size": 82.0},
    "perses_ddmin_xml": {"time": 1295.0, "size": 37.6},
    "perses_wdd_xml": {"time": 1273.0, "size": 37.5},
}

PAPER_ABSTRACT = {
    "hdd_time_improvement_pct": 51.31,
    "hdd_size_improvement_pct": 9.12,
    "perses_time_improvement_pct": 7.47,
    "perses_size_improvement_pct": 0.96,
}
...
def dedupe_keep_last(rows: list[dict[str, str]]) -> tuple[list[dict[str, str]], list[str]]:
    """Mirror the row selection needed to recover the released summary.

    The released hdd_ddmin_c.csv contains clang-22337 twice. Its README mean
    matches the later row, so we keep the final occurrence and expose the
    duplicate rather than silently averaging 33 rows.
    """
...
def summarize_rq2() -> dict:
    ...
    for name in RQ2_FILES:
        ...
        summaries[name] = {
            ...
            "unique_subjects": len(rows),
            "duplicates": duplicates,
            "mean_time_s": mean(rows, "Time"),
            "mean_tokens_remaining": mean(rows, "Token_remaining"),
        }

    # Validate the public README's rounded aggregate table.
    table_checks = {}
    for name, expected in EXPECTED_TABLE.items():
        got_t = summaries[name]["mean_time_s"]
        got_s = summaries[name]["mean_tokens_remaining"]
        # README rounds integer-looking values to nearest integer and reports
        # XML Perses size with one decimal place.
        t_ok = abs(round(got_t) - expected["time"]) <= 1.0
        if expected["size"] % 1:
            s_ok = abs(round(got_s, 1) - expected["size"]) <= 0.1
        else:
            s_ok = abs(round(got_s) - expected["size"]) <= 1.0
    ...
    # Pair subjects and compute mean per-subject percentage improvement across
    # the 32 C + 30 XML benchmarks; report alongside the paper abstract.
    per_subject = {}
    for system in ("hdd", "perses"):
        time_deltas = []
        size_deltas = []
        for lang in ("c", "xml"):
            b = normalized[f"{system}_ddmin_{lang}"]
            w = normalized[f"{system}_wdd_{lang}"]
            common = sorted(set(b) & set(w))
            for subject in common:
                time_deltas.append(
                    pct_improvement(float(b[subject]["Time"]), float(w[subject]["Time"]))
                )
                size_deltas.append(
                    pct_improvement(
                        float(b[subject]["Token_remaining"]),
                        float(w[subject]["Token_remaining"]),
                    )
                )
        per_subject[system] = {
            "subjects": len(time_deltas),
            "mean_time_improvement_pct": sum(time_deltas) / len(time_deltas),
            "mean_size_improvement_pct": sum(size_deltas) / len(size_deltas),
        }

    return {
        "level": "L1-partial-RQ2",
        "upstream_commit": UPSTREAM_COMMIT,
        "summaries": summaries,
        "table_checks": table_checks,
        "per_subject_improvements": per_subject,
        "paper_abstract": PAPER_ABSTRACT,
    }


# ----- scoped fresh L2 mechanism probe -----

# Relative body sizes chosen to create highly non-uniform fragments, as in the
# paper's motivating setting. Weight itself is measured from generated C tokens.
...
def fragment(i: int, scale: int) -> str:
    # All fragments are valid independent function definitions. Padding changes
    # lexical weight while preserving semantics.
    pads = " + 0" * scale
...
def assemble(indices: list[int], fragments: dict[int, str]) -> str:
    body = "".join(fragments[i] for i in indices)
    # The fixed harness makes f3/f4/f8 semantically necessary.
    return (
        "#include <stdio.h>\n"
        + body
...
class CompileRunOracle:
    ...
    def __call__(self, indices: list[int]) -> bool:
        ...
        with tempfile.TemporaryDirectory(prefix="wdd-") as td:
            ...
            cc = subprocess.run(
                ["gcc", "-std=c11", "-O0", str(src), "-o", str(exe)],
...
def weighted_partition(items: list[int], n: int, weights: dict[int, int]) -> list[list[int]]:
    """Greedy contiguous partitioning toward approximately equal total weight."""
    if n <= 1:
        return [items[:]]
    remaining_total = sum(weights[x] for x in items)
    remaining_parts = n
    out: list[list[int]] = []
    cur: list[int] = []
    acc = 0
    for x in items:
        target = remaining_total / remaining_parts
        wx = weights[x]
...
def reduce_ddmin(items: list[int], oracle: CompileRunOracle, weights: dict[int, int] | None) -> list[int]:
    ...
    while len(current) >= 2:
        parts = (
            weighted_partition(current, n, weights)
            if weights is not None
            else equal_partition(current, n)
        )
        reduced = False
        for part in parts:
            removed = set(part)
    ...
    # Explicit 1-minimal pass, matching the Wddmin design requirement.
    changed = True
    while changed:
        changed = False
        for x in current[:]:
...
def run_l2() -> dict:
    ...
    weights = {i: token_weight(fragments[i]) for i in fragments}
    initial = list(fragments)

    plain_oracle = CompileRunOracle(fragments)
    plain = reduce_ddmin(initial, plain_oracle, None)

    weighted_oracle = CompileRunOracle(fragments)
    weighted = reduce_ddmin(initial, weighted_oracle, weights)

    if set(plain) != CRITICAL or set(weighted) != CRITICAL:
        raise AssertionError(f"unexpected minima: ddmin={plain}, Wddmin={weighted}")
    if not weighted_oracle(weighted):
        raise AssertionError("weighted final candidate does not satisfy compile+run property")

    return {
        "level": "scoped-L2-mechanism",
        "property": "gcc -std=c11 -O0 succeeds and executable prints 42/exits 0",
        "initial_fragments": len(initial),
        "critical_fragments": sorted(CRITICAL),
        "weights": weights,
...
def main() -> None:
    ...
    print(
        "Fresh scoped L2: ddmin queries="
        f"{l2['ddmin']['queries']}, Wddmin queries={l2['wddmin']['queries']}, "
        f"improvement={l2['query_improvement_pct']:.2f}%"
    )
