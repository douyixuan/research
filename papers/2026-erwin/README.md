...
- Pinned upstream commit used here: `00ed69597b8cfcd0dfff15e86bc22b0ba319134a`
- Current upstream package at that commit: `@__haoyang__/erwin` 1.3.7
- Paper evaluation implementation: Erwin 1.3.1
...
1. clone the official Erwin repository at a pinned commit;
2. install and build it on Node.js 20;
3. run bounded exhaustive generation for `type`, `loc`, and `scope` modes with a small bound;
4. collect the emitted Solidity programs;
5. compile the generated programs with `solcjs 0.8.20`, the grammar version named by the paper;
...
|---|---|---|
| Erwin version | 1.3.1 | upstream 1.3.7 at pinned commit |
...
- **Artifact drift:** the paper names Erwin 1.3.1, while the current repository is 1.3.7. A faithful L3 run needs the exact 1.3.1 snapshot or Zenodo artifact pinned to the paper.
- **Randomness:** the released CLI does not expose an obvious random seed flag. CI therefore verifies invariants and records counts rather than expecting bit-for-bit identical programs.
- **Compiler drift:** the paper tested native `solc`; this small CI lane uses `solcjs 0.8.20` to cheaply test generated-program validity. That is not a substitute for the paper's native compiler builds or SMT/model-checker configuration.
...
1. recover/pin the exact Erwin 1.3.1 artifact;
